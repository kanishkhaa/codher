...
const Sidebar = ({ isSidebarOpen, setSidebarOpen }) => {
  ...
  const navItems = [
    ...
    { icon: <Calendar size={20} />, label: "Prescription Analyzer", path: "/prescription" },
    { icon: <FileText size={20} />, label: "Reminders", path: "/reminders" },
          ...
          {navItems.map((item, index) => (
            <li key={index}>
              <button 
                ...
                className={`w-full flex items-center p-3 rounded-lg transition-all hover:bg-slate-800 text-left ${window.location.pathname === item.path ? 'bg-indigo-900/60 text-indigo-400 shadow-lg' : ''}`}
              >
